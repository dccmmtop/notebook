...
1. 减少网络开销：本来多次的网络请求的操作可以用一次请求完成，多次请求逻辑封装在 lua 脚本中，放在 redis 上执行一次，减少网络往返的开销，这一点和管道类似。
2. 原子操作： redis 会将整个 lua 脚本作为一个整体执行，是原子的。管道不是原子的，但是一些批量操作的命令是原子的，如 `mget`
3. 替代 redis 的事务功能： 到目前为止，redis 的事务功能依然很鸡肋，几乎不会使用。而 redis 的 lua 脚本几乎实现了常规的事务功能，redis 官方推荐可以用 lua 实现事务功能而不是用 redis 自带的。
...
`script` 参数是一段 lua 脚本， `numkeys` 用于指定键名参数的个数，键名参数 key 是从第 3 个参数开始算起的，表明在脚本中所用到的那些 key，这些键名参数在 Lua 脚本中通过全局变量 `KEYS` 数组访问，用 1 为基址的形式访问，如：`KEYS[1] KEYS[2]`, 除了 `numkeys` 个键名参数后，最后的附加参数 arg [arg ...] 可以在 Lua 脚本中通过全局变量 ARGV 数组访问，如 `ARGV[1] ARGV[2]`
