...
var VimImPlugin = class extends import_obsidian.Plugin {
  ...
  onVimModeChanged(modeObj) {
    console.log("当前模式是: " + modeObj.mode);
    switch (modeObj.mode) {
      case "insert":
        exec("im-select 2052", (error, stdout, stderr) => {
            if (error) {
              console.error(`switch error: ${error}`);
            }
            console.log(`switch  中文`);
          });
        break;
      case "normal":
          exec("im-select 1033", (error, stdout, stderr) => {
            if (error) {
              console.error(`switch error: ${error}`);
            }
            console.log(`switch  英文`);
          });
        break;
      default:
        break;
